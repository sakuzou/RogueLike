...
public abstract class MovingObject : MonoBehaviour
{
    ...
    private int Direction_X = 0; //キャラの向いている方向(X軸方向)
    private int Direction_Y = 0; //キャラの向いている方向(Y軸方向)
    ...
    protected bool Move(int xDir, int yDir, out RaycastHit2D hit)
    {
        ...
        boxCollider.enabled = false;                            //自身のColliderを無効にし、Linecastで自分自身を判定しないようにする
        hit = Physics2D.Linecast(start, end, blockingLayer);    //現在地と目的地との間にblockingLayerのついたオブジェクトが無いか判定
        boxCollider.enabled = true;                             //Colliderを有効に戻す
    ...
    protected virtual void Attack<T>() where T : Component
    {
        ...
        Vector2 start = transform.position;                                   //現在地を取得       
        Vector2 end = start + new Vector2(Direction_X, Direction_Y);          //目的地を取得
    ...
    protected virtual void DirectionChange(int xDir,int yDir)
    {
        //向いている方向を求める
        Direction_X = xDir;
        Direction_Y = yDir;
